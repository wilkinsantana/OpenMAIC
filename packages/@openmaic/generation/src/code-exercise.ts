...
export function renderCodeExerciseHtml(config: Partial<CodeExercise>, legacyHtml?: string): string {
  ...
  *{box-sizing:border-box}html,body{margin:0;height:100%;font:14px/1.5 system-ui;color:#e2e8f0;background:#101827}body{display:flex;flex-direction:column;overflow:hidden}
  ...
  #description{padding:12px}.test{padding:10px 12px;border-bottom:1px solid #334155;white-space:pre-wrap}.test:last-child{border:0}.test[data-state=passed]{border-left:3px solid #34d399}.test[data-state=failed]{border-left:3px solid #fb7185}
  ...
  <aside class="pane"><section class="card" id="instructions-card"><h2>Instructions</h2><p id="description"></p></section><section class="card" id="preview-card" hidden><h2>Live preview</h2><iframe id="preview" sandbox="allow-scripts" title="Exercise preview"></iframe></section><section class="card"><h2 id="test-heading">Verification tests</h2><div id="tests"></div></section><section class="card output-panel"><h2>Execution output</h2><pre id="output" aria-live="polite">Run the code to see results.</pre></section><section class="card" id="legacy-card" hidden><button id="legacy-toggle">Open original interactive preview</button><iframe id="legacy-frame" title="Original exercise runtime" sandbox="allow-scripts" hidden></iframe></section></aside></main>
